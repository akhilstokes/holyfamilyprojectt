...
const DeliveryDashboard = () => {
  ...
  const deliverToLab = async (t) => {
    ...
    if (sellReqId) {
      try { await fetch(`${API}/api/sell-requests/${sellReqId}/deliver-to-lab`, { method: 'PUT', headers: authHeaders() }); } catch {}
    }
  ...
  const setBarrelIds = async (t) => {
    ...
    try { await updateStatus(t._id, { meta: { barrels: ids } }); await load(); } catch (e) { alert(e?.message || 'Failed to save barrel IDs'); }
  };

  const openLabCheckIn = (t) => {
    const customer = t?.customerUserId?.name || t?.customerUserId?.email || '';
    const count = t?.meta?.barrelCount ?? '';
    const receivedAt = new Date().toISOString().slice(0,16);
    const sampleId = t?.meta?.sellRequestId || t?._id;
    const qs = new URLSearchParams({ sampleId: String(sampleId), customerName: String(customer), barrelCount: String(count), receivedAt });
    const barrelIds = Array.isArray(t?.meta?.barrels) ? t.meta.barrels : [];
    barrelIds.forEach(id => { if (id) qs.append('barrels', String(id)); });
    navigate(`/delivery/lab-check-in?${qs.toString()}`);
  };
