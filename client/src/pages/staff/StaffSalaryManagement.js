...
const StaffSalaryManagement = () => {
  ...
  const loadAttendanceData = useCallback(async () => {
    try {
      const res = await fetch(`${base}/api/attendance/staff?year=${selectedYear}&month=${selectedMonth}`, { headers });
      ...
      if (res.ok) {
        ...
        if (data.data) {
          setCalculationForm(prev => ({
            ...
            dailyWage: data.data.dailyWage || 0
          }));
        }
      }
    } catch (err) {
  ...
  const calculateSalary = () => {
    const { dailyWage, workingDays, overtimeHours, overtimeRate, allowances, deductions, bonuses } = calculationForm;
    
    // Calculate base salary
    const baseSalary = workingDays * dailyWage;
    
    // Calculate overtime pay (assuming 8 hours per day)
    ...
    try {
      ...
      await fetch(`${base}/api/notifications/staff-trip-event`, {
        ...
        body: JSON.stringify({
          ...
          message: `Staff member has requested salary review for ${calculationForm.month}/${calculationForm.year}. Net Amount: ₹${calculatedSalary.netSalary.toFixed(2)}`,
          ...
          meta: {
            ...
            workingDays: calculatedSalary.workingDays,
  ...
  const downloadPayslip = async (salaryId) => {
    try {
      const res = await fetch(`${base}/api/salary/payslip/${salaryId}`, { headers });
  ...
  useEffect(() => {
    loadCurrentSalary();
    loadAttendanceData();
  }, [loadCurrentSalary, loadAttendanceData]);
    ...
    <div style={{ padding: 16 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
        <div>
          ...
          <p style={{ margin: '8px 0 0 0', color: '#64748b' }}>View and track your daily wage salary</p>
        </div>
        ...
        <div className="dash-card" style={{ padding: 20, marginBottom: 20 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 16 }}>
            ...
            <div style={{ textAlign: 'center', padding: 16, backgroundColor: '#fef3c7', borderRadius: 8 }}>
              <div style={{ fontSize: 14, color: '#64748b', marginBottom: 4 }}>Working Days</div>
              <div style={{ fontSize: 24, fontWeight: 700, color: '#d97706' }}>
                {currentSalary.workingDays || 0}
              </div>
            </div>
            <div style={{ textAlign: 'center', padding: 16, backgroundColor: '#f0fdf4', borderRadius: 8 }}>
              <div style={{ fontSize: 14, color: '#64748b', marginBottom: 4 }}>Daily Wage</div>
              <div style={{ fontSize: 24, fontWeight: 700, color: '#16a34a' }}>
                ₹{currentSalary.dailyWage?.toFixed(2) || '0.00'}
              </div>
            </div>
          </div>
        </div>
      ...
      {attendanceData && (
        <div className="dash-card" style={{ padding: 20, marginBottom: 20 }}>
          <h3 style={{ margin: '0 0 16px 0', color: '#1e293b' }}>Attendance Summary</h3>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(150px, 1fr))', gap: 16 }}>
            <div style={{ textAlign: 'center', padding: 12, backgroundColor: '#f0fdf4', borderRadius: 8 }}>
              <div style={{ fontSize: 12, color: '#64748b', marginBottom: 4 }}>Working Days</div>
              <div style={{ fontSize: 20, fontWeight: 600, color: '#16a34a' }}>
                {attendanceData.workingDays || 0}
              </div>
            </div>
            <div style={{ textAlign: 'center', padding: 12, backgroundColor: '#fef2f2', borderRadius: 8 }}>
              <div style={{ fontSize: 12, color: '#64748b', marginBottom: 4 }}>Absent Days</div>
              <div style={{ fontSize: 20, fontWeight: 600, color: '#dc2626' }}>
                {attendanceData.absentDays || 0}
              </div>
            </div>
            <div style={{ textAlign: 'center', padding: 12, backgroundColor: '#fef3c7', borderRadius: 8 }}>
              <div style={{ fontSize: 12, color: '#64748b', marginBottom: 4 }}>Overtime Hours</div>
            ...
            <div style={{ textAlign: 'center', padding: 12, backgroundColor: '#f0f9ff', borderRadius: 8 }}>
              ...
              <div style={{ fontSize: 20, fontWeight: 600, color: '#3b82f6' }}>
                {attendanceData.attendancePercentage?.toFixed(1) || '0.0'}%
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="dash-card" style={{ padding: 20, marginBottom: 20 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 16, marginBottom: 20 }}>
          ...
          <div>
            ...
            <input
              type="number"
              value={calculationForm.dailyWage}
              onChange={(e) => setCalculationForm(prev => ({ ...prev, dailyWage: Number(e.target.value) }))}
              style={{ width: '100%', padding: '8px 12px', borderRadius: 6, border: '1px solid #d1d5db' }}
            />
          </div>
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) => setCalculationForm(prev => ({ ...prev, workingDays: Number(e.target.value) }))}
              style={{ width: '100%', padding: '8px 12px', borderRadius: 6, border: '1px solid #d1d5db' }}
            />
          </div>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              <thead>
                <tr style={{ backgroundColor: '#f8fafc' }}>
                  ...
                  <th style={{ padding: 12, textAlign: 'right', borderBottom: '1px solid #e2e8f0' }}>Working Days</th>
                  <th style={{ padding: 12, textAlign: 'right', borderBottom: '1px solid #e2e8f0' }}>Daily Wage</th>
                  ...
                  <th style={{ padding: 12, textAlign: 'center', borderBottom: '1px solid #e2e8f0' }}>Actions</th>
                </tr>
              </thead>
              <tbody>
                {salaryHistory.map((salary, index) => (
                  <tr key={salary._id} style={{ backgroundColor: index % 2 === 0 ? '#fff' : '#f8fafc' }}>
                    ...
                    <td style={{ padding: 12, textAlign: 'right', borderBottom: '1px solid #e2e8f0' }}>
                      ₹{salary.dailyWage?.toFixed(2) || '0.00'}
                    </td>
                    <td style={{ padding: 12, textAlign: 'right', borderBottom: '1px solid #e2e8f0' }}>
